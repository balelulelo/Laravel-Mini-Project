<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-bg-light overflow-hidden shadow-sm sm:rounded-lg mb-6">
            ...
            <div class="bg-bg-light overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6 text-gray-800">
                    ...
                    @if ($user->studyprofile)
                                ...
                                @if ($user->studyprofile->subjects->isNotEmpty())
                                    <div class="flex flex-wrap gap-1 mt-1">
                                        @foreach ($user->studyprofile->subjects as $subject)
                                            <span class="inline-block bg-primary/20 text-primary-800 text-xs font-medium px-2 py-0.5 rounded-full">
                                                {{ $subject->name }}
                                            </span>
                                        @endforeach
                                    </div>
                        ...
                        <a href="{{ route('profile.study.edit') }}" class="text-primary hover:text-primary-700 hover:underline text-sm">
                            Edit Your Study Profile
                        </a>
                    @else
                        <p class="text-gray-500">You haven't set up your study profile yet.</p>
                         <a href="{{ route('profile.study.edit') }}" class="text-primary hover:text-primary-700 hover:underline text-sm mt-2 inline-block">
                            Create Study Profile
                        </a>
                    @endif
                </div>
            </div>
            <div class="bg-bg-light overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-800">
                                ...
                                <div class="flex items-center justify-between p-3 border border-gray-200 rounded-md">
                                    ...
                                    <div>
                                        @if ($connectedUser->studyprofile)
                                            <a href="{{ route('partners.show', ['studyProfile' => $connectedUser->studyprofile->id]) }}" class="text-sm text-primary hover:text-primary-700 hover:underline mr-4">
                                                View Profile
                                            </a>
                                        @endif
                    ...
                    @else
                        ...
                        <a href="{{ route('partners.index') }}" class="text-primary hover:text-primary-700 hover:underline text-sm mt-2 inline-block">
                            Find Partners
                        </a>
                    @endif
                </div>
